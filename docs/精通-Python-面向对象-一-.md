...
![比较操作符方法](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_02_01.jpg)![比较操作符方法](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_02_02.jpg)![比较操作符方法](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_02_03.jpg)![比较操作符方法](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_02_04.jpg)
...
第一个修改是使用**分而治之**的设计策略。之前的版本将![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_01.jpg)分成*n*步；循环执行*n*个单独的乘法操作。如果我们能找到一种方法将问题分成两个相等的部分，问题就会分解成![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_02.jpg)步。给定`pow1(2,1024)`，`Power1`可调用对象执行了 1024 次乘以 2 的计算。我们可以将这个优化到 10 次乘法，显著提高速度。

而不是简单地乘以一个固定值，我们将使用“快速指数”算法。它使用三个计算![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_01.jpg)的基本规则，如下所示：

+   如果![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_03.jpg)：![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_04.jpg)，结果就是 1。

+   如果*n*为奇数且![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_05.jpg)，结果是![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_06.jpg)。这涉及到![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_07.jpg)的递归计算。这仍然进行了乘法，但并非真正的优化。

+   如果*n*是偶数且![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_08.jpg)，结果是![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_09.jpg)。这涉及到![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_10.jpg)的递归计算。这将乘法次数减半。
...
我们对输入值应用了三条规则。如果*n*为零，我们将返回 1。如果*n*为奇数，我们将进行递归调用并返回![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_06.jpg)。如果*n*为偶数，我们将进行递归调用并返回![提高性能](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_09.jpg)。
...
如果之前已经请求过![使用备忘录或缓存](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_01.jpg)的值，那么将返回该结果，不进行计算。这就是我们之前所说的巨大加速。

否则，必须计算并保存![使用备忘录或缓存](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_05_01.jpg)的值在备忘录缓存中。使用三条规则来计算快速指数，以获取和放置缓存中的值。这确保了未来的计算将能够利用缓存的值。
...
我们可以将结果累积到内置的`list`类中。我们可以通过![定义一种新类型的序列](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_06_01.jpg)来计算均值，其中 N 是*x*中的元素数：
...
标准偏差可以通过![定义一种新类型的序列](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_06_02.jpg)来计算：
...
当我们移除一个项目，也就是在成功的`pop()`操作之后，我们必须调整我们的总和。当我们添加一个项目（初始时或通过`insert()`方法），我们也必须调整我们的总和。我们需要实现的其他方法将利用这两种方法来确保这三个总和不变。我们保证 L.sum0 总是![选择急切与懒惰的计算](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_06_03.jpg)，sum1 总是![选择急切与懒惰的计算](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_06_04.jpg)，sum2 总是![选择急切与懒惰的计算](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_06_05.jpg)。
...
不要对这种事情感到惊讶。以下是使用![数字的 ABC](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_01.jpg)近似的一个例子：
...
`__rsub__()`操作是“反射减法”。`X.__sub__(Y)`操作是预期的![方法解析和反射运算符概念](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_02.jpg)。`A.__rsub__(B)`操作是反射![方法解析和反射运算符概念](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_03.jpg)；实现方法来自右操作数的类。我们已经看到了以下两条规则：
...
理想情况下，缩放因子是 10 的幂，但我们并没有明确强制执行这一点。跟踪缩放幂和与幂相关的比例因子是一个相对简单的扩展。我们可以将 2 存储为幂，并将![Creating a numeric class](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_04.jpg)存储为因子。我们已经简化了这个类的定义，只需跟踪因子。
...
定义新类别数字的整个原因是为了重载算术运算符。每个`FixedPoint`对象有两部分：`value`和`scale`。我们可以这样说：![定义 FixedPoint 二进制算术运算符](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_05.jpg)。

请注意，我们已经使用正确但低效的浮点表达式在下面的示例中解出了代数。我们将讨论稍微更有效的纯整数操作。

加法（和减法）的一般形式是这样的：![定义 FixedPoint 二进制算术运算符](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_06.jpg)。但它创建了一个有很多无用精度的结果。

想象一下添加 9.95 和 12.95。我们将（原则上）有 229000/10000。这可以正确地减少为 2290/100。问题是它也减少为 229/10，这不再是分。我们希望避免以一般方式减少分数，而尽可能坚持分或毫。

我们可以确定![定义 FixedPoint 二进制算术运算符](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_07.jpg)有两种情况：

+   **比例因子匹配**：在这种情况下，总和是![定义 FixedPoint 二进制算术运算符](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_08.jpg)。当添加`FixedPoint`和普通整数时，这也可以工作，因为我们可以强制普通整数具有所需的比例因子。

+   **比例因子不匹配**：正确的做法是产生一个具有两个输入值的最大比例因子的结果，![定义 FixedPoint 二进制算术运算符](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_09.jpg)。从这里，我们可以计算两个比例因子，![定义 FixedPoint 二进制算术运算符](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_10.jpg)和![定义 FixedPoint 二进制算术运算符](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_11.jpg)。其中一个比例因子将是 1，另一个将小于 1。我们现在可以用一个公共值在分母上相加。代数上，它是![定义 FixedPoint 二进制算术运算符](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_12.jpg)。这可以进一步优化为两种情况，因为一个因子是 1，另一个是 10 的幂。

我们实际上不能优化乘法。它本质上是![定义 FixedPoint 二进制算术运算符](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_13.jpg)。当我们相乘`FixedPoint`值时，精度确实会增加。

除法是乘以倒数，![定义 FixedPoint 二进制算术运算符](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_07_14.jpg)。如果 A 和 B 具有相同的比例，这些值将取消，以便我们确实有一个方便的优化可用。然而，这将把比例从分变为整，这可能不合适。前向运算符，围绕类似的样板构建，看起来像这样：
...
第二阶段是将装饰器应用于原始定义。当我们将装饰器(`@d`)应用于函数(`F`)时，效果就好像我们创建了一个新函数，![构建函数](https://github.com/OpenDocCN/freelearn-python-pt2-zh/raw/master/docs/ms-oo-py/img/0971OS_08_01.jpg)。名称是相同的，但功能可能会有所不同，具体取决于已添加、删除或修改的功能类型。然后，我们将编写以下代码：
